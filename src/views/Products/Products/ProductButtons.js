...
const UpdateLink = props => (
  <>
<Link to={{pathname: `/goods/updateproduct/${props.product_id}`, updateproduct_id: props.product_id}}>
<Button id="caret" color="secondary"> <i className="fa fa-pencil"></i></Button>
</Link>
  </>
)

class UpdateDeleteProducts extends Component {
  ...
  DeleteClass = (e) => {
    ...
    e.preventDefault()
      httpDelete({
        url:`api/admin/product/delete/${this.props.id}` 
      })
      .then(response => {
