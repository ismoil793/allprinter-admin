...
const UpdateLink = props => (
  <>
<Link to={{pathname: `/task/${props.task_id}`, task_id: props.task_id}}>
...
class UpdateDelete extends Component {
  ...
  Delete = (e) => {
    ...
    e.preventDefault()
      httpDelete({
        url:`api/admin/task/delete/${this.props.id}` 
      })
      .then(response => {
     
        notyf.error('Вы удалили задачу')
        this.props.function()
      })
      .catch(error =>{
        console.log(error)
      ...
      <div className="animated fadeIn">
        <Row>
         ...
         <ButtonDropdown className="mr-1" isOpen={this.state.dropdownOpen[10]} toggle={() => { this.toggle(10); }}>
                  <UpdateLink task_id={this.props.id} />
