...
const UpdateLink = props => (
  <>
<Link to={{pathname: `/catalog/updatebrand/${props.brand_id}`, brand_id: props.brand_id}}>
<Button id="caret" color="secondary"> <i className="fa fa-pencil"></i></Button>
</Link>
  </>
)

class UpdateDeleteBrands extends Component {
  ...
  DeleteClass = (e) => {
    ...
    e.preventDefault()
      httpDelete({
        url:`api/admin/brand/delete/${this.props.id}` 
      })
      .then(response => {
     
        notyf.error('Вы удалили бренд')
        this.props.function()
      })
      .catch(error =>{
        console.log(error)
      ...
      <div className="animated fadeIn">
        <Row>
         ...
         <ButtonDropdown className="mr-1" isOpen={this.state.dropdownOpen[10]} toggle={() => { this.toggle(10); }}>
                  <UpdateLink brand_id={this.props.id} />
...
export default UpdateDeleteBrands;
